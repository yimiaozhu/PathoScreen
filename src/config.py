...
PATHWAY_ARCHITECTURES: Dict[int, PathwayConfig] = {
...
def get_config(pathway_id: int) -> PathwayConfig:
    if pathway_id not in PATHWAY_ARCHITECTURES:
        raise ValueError(f"Unknown pathway_id: {pathway_id}")
    return PATHWAY_ARCHITECTURES[pathway_id]
